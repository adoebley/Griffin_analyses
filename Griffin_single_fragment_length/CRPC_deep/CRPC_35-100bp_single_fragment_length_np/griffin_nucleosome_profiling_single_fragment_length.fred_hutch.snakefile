...
snakemake -s griffin_nucleosome_profiling_single_fragment_length.fred_hutch.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName}" -j 40 -np
...
snakemake -s griffin_nucleosome_profiling_single_fragment_length.fred_hutch.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p restart-new -q restart-new --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName} --requeue" -j 40 -np
"""
...
rule calc_cov:
	...
	params:
		...
		griffin_coverage_script = config['griffin_scripts_dir']+'/griffin_coverage_single_fragment_length.py',
