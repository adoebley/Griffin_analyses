...
snakemake -s griffin_GC_and_mappability_correction_single_fragment_length.fred_hutch.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName}" -j 40 -np

#command to run on restart
#need to add '-q restart-new' according to scicomp. This should be fixed at some point.
snakemake -s griffin_GC_and_mappability_correction_single_fragment_length.fred_hutch.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p restart-new -q restart-new --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName} --requeue" -j 40 -np
"""
...
rule all:
	input: 
		...
		expand("{out_dir}/GC_plots/{samples}.GC_bias.pdf", samples=config["samples"], out_dir=config['out_dir']),
		#
...
rule GC_counts:
	...
	params:
		...
		griffin_GC_counts_script = config['griffin_scripts_dir']+'/griffin_GC_counts_single_fragment_length.py'
...
rule GC_bias:
	...
	output:
		...
		GC_plots_file = "{out_dir}/GC_plots/{samples}.GC_bias.pdf"
	params:
		sample_name = "{samples}",
		...
		size_range=config['GC_bias_size_range'],
		fragment_length = config['fragment_length'],
		griffin_GC_bias_script = config['griffin_scripts_dir']+'/griffin_GC_bias_single_fragment_length.py'
