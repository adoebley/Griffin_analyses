...
# GC_bias_path = '../../MBC_GC_correction/griffin_single_fragment_length_GC_and_mappability_correction/results/GC_bias/MBC_1041_1_ULP.GC_bias.txt'
# mappability_bias_path = '../../MBC_GC_correction/griffin_single_fragment_length_GC_and_mappability_correction/results/mappability_bias/MBC_1041_1_ULP.mappability_bias.txt'
...
# sites_yaml = '/fh/fast/ha_g/user/adoebley/projects/griffin_revisions_1/MBC/ATAC_nucleosome_profiling_with_CNA_correction_50kb/config/sites.yaml'
# griffin_scripts_dir = '../'
...
#Keep this because it's a quick way to get the right format
        ...
        if abs(read.template_length)>=sz_range[0] and abs(read.template_length)<=sz_range[1]         and read.is_paired==True and read.mapping_quality>=map_q and read.is_duplicate==False and read.is_qcfail==False:
            ...
            if read.is_reverse==False and read.template_length>0:
                ...
                if midpoint>=start and midpoint<end:
                    ...
                    ##get the GC bias
                    #modified for a single fragment length GC bias
                    read_GC_fraction = sum(fragment_seq)/len(fragment_seq)
                    read_GC_content = np.int32(np.round((read_GC_fraction)*fragment_length))
                    #print(sum(fragment_seq),len(fragment_seq), read_GC_fraction, read_GC_content)
                    read_GC_bias = GC_bias[fragment_length][read_GC_content]
...
#     results = collect_fragments(to_fetch.iloc[i].values)
#     break
