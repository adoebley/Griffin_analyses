...
#AL - also had to do additional modification to make this compatable with python3 (noted below)
...
for read in bamF.fetch():
	...
	if not reverse:
		...
		#AL modified due to issue with unmapped reads failing
		try:
			start = read_end - args.length - ((166-args.length) // 2)
			if start < 1:
				start = 1
		except:
			#AL modified these lines to make them compatable with python3 for the griffin paper version
			print ('\n\nerror flag:'+str(read.flag)+' chr:'+str(read.reference_name)+' start:'+str(read_start)+' end:'+str(read_end)+' read_len:'+str(read.reference_length))
			print (str(read)+'\n')
		#end AL mods
