...
option_list <- list(
  ...
  make_option(c("--genomeStyle"), type = "character", default = "NCBI", help = "NCBI or UCSC chromosome naming convention; use UCSC if desired output is to have \"chr\" string. [Default: %default]"),
	...
	make_option(c("--maleChrXLogRThres"), type="numeric", default=-0.50, help = "ChrX Log ratio threshold to confirm as male gender."), #AL changed default from 0.8 to 0.5
	make_option(c("--fracReadsInChrYForMale"), type="numeric", default=0.001, help = "Threshold for fraction of reads in chrY to assign as male. Default: [%default]"),#AL added, default is correct here
...
fracReadsInChrYForMale <- opt$fracReadsInChrYForMale #AL added
...
for (i in 1:length(files)){
	...
	normal_counts <- loadReadCountsFromWig(normal_reads, chrs=chrs, gc=gc, map=map, 
					centromere=centromere, targetedSequences=targetedSequences, genomeStyle=genomeStyle, 
					chrNormalize = chrNormalize, fracReadsInChrYForMale = fracReadsInChrYForMale,
					chrXMedianForMale = maleChrXLogRThres) #AL added genomeStyle, chrNormalize, fracReadsInChrYForMale and chrXMedianForMale
	
	gender <- normal_counts$gender
	message("Gender ", gender) #AL added
